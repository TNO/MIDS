...
/**
 * Represent a composition of two deterministic acceptors with defined input alphabets.
 *
 ...
 */
public class DetAcceptorCompositionWithInput<S1, S2, I,
...
{
    /**
     * Construct a composition of two acceptors with a given rule for combining accepting states.
     * 
     * @param ts1 First acceptor.
     * @param ts2 Second acceptor.
     * @param combiner Rule for combining accepting states.
     */
    public DetAcceptorCompositionWithInput(A1 ts1, A2 ts2, AcceptanceCombiner combiner) {
        super(ts1, ts2, combiner);
    }
    ...
    public Pair<S1, S2> getTransition(Pair<S1, S2> state, I input) {
        ...
        S1 t1 = ((s1 == null) || !ts1.getInputAlphabet().containsSymbol(input)) ? null : ts1.getTransition(s1, input);
        ...
        S2 t2 = ((s2 == null) || !ts2.getInputAlphabet().containsSymbol(input)) ? null : ts2.getTransition(s2, input);
